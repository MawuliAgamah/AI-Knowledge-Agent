...
def nebula_create_space(space_name: str):
    ...
    with connection_pool.session_context('root', 'nebula') as session:
        # create a space
        session.execute(
            f"CREATE SPACE IF NOT EXISTS `{space_name}` (vid_type=FIXED_STRING(256), partition_num=1);")
...
def construct_knowledge_graph_index(documents, storage_context, space_name, edge_types, rel_prop_names, tags):
    my_file = Path(
        "/Users/mawuliagamah/gitprojects/STAR/data/graph/test/default__vector_store.json")
    if my_file.is_file():
        ...
        kg_index.storage_context.persist(
            persist_dir="/Users/mawuliagamah/gitprojects/STAR/data/graph/test/")
        return kg_index
...
if __name__ == '__main__':
    ...
    document = get_document_from_file(
        file_path="/Users/mawuliagamah/gitprojects/STAR/data/documents/word")
    llm = OpenAI(temperature=0, model="gpt-3.5-turbo-instruct")
    service_context = ServiceContext.from_defaults(llm=llm, chunk_size=512)
    kg_index = construct_knowledge_graph_index(
        documents=document, space_name="STAR", storage_context=storage_context, edge_types=edge_types, tags=tags, rel_prop_names=rel_prop_names)
