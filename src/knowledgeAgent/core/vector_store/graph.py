...
class NebulaDB:
    ...
    def create_graph_space(self,space_name):
        ...
        with connection_pool.session_context('root', 'nebula') as session:
            # create a space
            session.execute(f"CREATE SPACE IF NOT EXISTS `{space_name}` (vid_type=FIXED_STRING(256), replica_factor=1, partition_num=1);")
            session.execute(f"USE {space_name};")
            for entity in ["CONCEPT", "TOPIC", "EXAMPLE", "REFERENCE", "EXPLANATION"]:
                session.execute(f"""
...
class LlamaIndexKG:
    ...
    def construct_property_graph_index(self,llm,space_name,documents,
        ...
        kg_extractor = SchemaLLMPathExtractor(
            ...
            kg_validation_schema=validation_schema_rules,
            strict=True
            #num_workers=4,
...
if __name__ == '__main__':
    ...
    nebula.create_graph_space(space_name = "Knowledge_Graph_2")
