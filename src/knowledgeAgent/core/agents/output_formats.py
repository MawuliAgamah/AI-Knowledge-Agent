...
#    review : str = Field(description= "Analysis and review of the task")
    ...
    improved_task: str = Field(
        description="New version of the sub-task based on suggested improvements to be given to the next AI agent to execute.")
...
class SubTask(BaseModel):
    ...
    sub_task: str = Field(
        description="description of the sub task to be completed")
...
class Task(BaseModel):
    ...
    subtasks: List[SubTask] = Field(
        description="list of sub tasks to be completed")
...
class Thought(BaseModel):
    ...
    thought: str = Field(
        description="A 'thought', which describes the an agents 'thinking' to solve the problem")
    reasoning: str = Field(
        description="Explanation of WHY this approach was taken to solve the problem.")
...
class Thoughts(BaseModel):
    ...
    thought: List[Thought] = Field(
        description="An array of thoughts detailing your reasoning, solution preparation, and next steps.")
...
class Interpretation(BaseModel):
    ...
    """
    id: str = Field(
        description="A unique identifier for each analysis and reasoning block.")
    analysis: str = Field(
        description="A detailed review and analysis of the task the model was given, including an evaluation of its approach and outcome.")
    questions: str = Field(
        description="Any additional questions or clarifications needed to improve the model's performance or better understand the task.")
...
class interpretationFormat(BaseModel):
    ...
    """
    thought: List[Interpretation] = Field(
        description="A list of at maxiumum 8 interprations")
...
class BreakDownTask(BaseModel):
    ...
    task_breakdown: List[str] = Field(
        description="Breakdown of the task handed to agent")
...
class BasicAgentOutputFormat(BaseModel):
    ...
    """
    response: str = Field(
        description="Response to the prompt given to the agent")
